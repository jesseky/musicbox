...
class Ui:
    ...
    def build_playinfo(self, song_name, artist, album_name, pause=False):
        ...
        self.screen.clrtoeol()
        if pause:
            self.screen.addstr(1, 1, '_ _ z Z Z', curses.color_pair(3))
        else:
            self.screen.addstr(1, 1, '♫  ♪ ♫  ♪', curses.color_pair(3))
        self.screen.addstr(1, self.posLeft, song_name + '   -   ' + artist + '  < ' + album_name + ' >', curses.color_pair(4))
        self.screen.refresh()

    def build_loading(self):
        self.screen.addstr(6, self.posLeft, '享受高品质音乐，loading...', curses.color_pair(1))
        self.screen.refresh()
    ...
    def build_menu(self, datatype, title, datalist, offset, index, step):
        ...
        self.screen.addstr(4, self.posLeft, title, curses.color_pair(1))

        if len(datalist) == 0:
            self.screen.addstr(8, self.posLeft, '这里什么都没有 -，-')

        else:
            if datatype == 'main':
                for i in range(offset, min(len(datalist), offset + step)):
                    if i == index:
                        self.screen.addstr(i - offset + 8, self.posLeft-3, '-> ' + str(i) + '. ' + datalist[i],
                                           curses.color_pair(2))
                    else:
                        self.screen.addstr(i - offset + 8, self.posLeft, str(i) + '. ' + datalist[i])

            elif datatype == 'songs':
                for i in range(offset, min(len(datalist), offset + step)):
                    # this item is focus
                    if i == index:
                        self.screen.addstr(i - offset + 8, self.posLeft-3,
                                           '-> ' + str(i) + '. ' + datalist[i]['song_name'] + '   -   ' + datalist[i][
                                               'artist'] + '  < ' + datalist[i]['album_name'] + ' >',
                                           curses.color_pair(2))
                    else:
                        self.screen.addstr(i - offset + 8, self.posLeft,
                                           str(i) + '. ' + datalist[i]['song_name'] + '   -   ' + datalist[i][
                                               'artist'] + '  < ' + datalist[i]['album_name'] + ' >')
                ...
                for i in range(offset, min(len(datalist), offset + step)):
                    if i == index:
                        self.screen.addstr(i - offset + 8, self.posLeft-3, '-> ' + str(i) + '. ' + datalist[i]['title'],
                                           curses.color_pair(2))
                    else:
                        self.screen.addstr(i - offset + 8, self.posLeft, str(i) + '. ' + datalist[i]['title'])
                ...
                for i in range(offset, min(len(datalist), offset + step)):
                    if i == index:
                        self.screen.addstr(i - offset + 8, self.posLeft-3, '-> ' + str(i) + '. ' + datalist[i]['song_name'],
                                           curses.color_pair(2))
                    else:
                        self.screen.addstr(i - offset + 8, self.posLeft, str(i) + '. ' + datalist[i]['song_name'])
                ...
                for i in range(offset, min(len(datalist), offset + step)):
                    if i == index:
                        self.screen.addstr(i - offset + 10, self.posLeft-3, '-> ' + str(i) + '.' + datalist[i - 1],
                                           curses.color_pair(2))
                    else:
                        self.screen.addstr(i - offset + 10, self.posLeft, str(i) + '.' + datalist[i - 1])
            elif datatype == 'help':
                for i in range(offset, min(len(datalist), offset + step)):
                    if i == index:
                        self.screen.addstr(i - offset + 8, self.posLeft-3,
                        ...
                        self.screen.addstr(i - offset + 8, self.posLeft,
                                           ...
                                           datalist[i][2])
                self.screen.addstr(20, 1, 'NetEase-MusicBox 基于Python，所有版权音乐来源于网易，本地不做任何保存')
                self.screen.addstr(21, 1, '按 [G] 到 Github 了解更多信息，帮助改进，或者Star表示支持~~')
                self.screen.addstr(22, self.posLeft, 'Build with love to music by omi')
    ...
    def build_login_bar(self):
        ...
        self.screen.move(4, 1)
        self.screen.clrtobot()
        self.screen.addstr(5, self.posLeft, '请输入登录信息(支持手机登陆)',curses.color_pair(1))
        self.screen.addstr(8, self.posLeft, "账号:", curses.color_pair(1))
        self.screen.addstr(9, self.posLeft, "密码:", curses.color_pair(1))
    ...
    def build_login_error(self):
        self.screen.move(4, 1)
        self.screen.clrtobot()
        self.screen.addstr(8, self.posLeft, '艾玛，登录信息好像不对呢 (O_O)#', curses.color_pair(1))
        self.screen.addstr(10, self.posLeft, '[1] 再试一次')
        self.screen.addstr(11, self.posLeft, '[2] 稍后再试')
        self.screen.addstr(14, self.posLeft, '请键入对应数字:', curses.color_pair(2))
        self.screen.refresh()
        x = self.screen.getch()
        return x
    ...
    def get_param(self, prompt_string):
        ...
        self.screen.move(4, 1)
        self.screen.clrtobot()
        self.screen.addstr(5, self.posLeft, prompt_string, curses.color_pair(1))
        self.screen.refresh()
        info = self.screen.getstr(10, self.posLeft, 60)
        if info.strip() is '':
